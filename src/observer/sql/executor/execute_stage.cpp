...
RC ExecuteStage::do_select(SQLStageEvent *sql_event)
{
  ...
  std::string table_name = select_stmt->tables().front()->name();
  if (std::find(all_tables.begin(), all_tables.end(), table_name) == all_tables.end())
  ...
  // check columns name
  std::unordered_set<std::string> want_relations;
  const auto &selection = query->sstr.selection;
  for (size_t i = 0; i < selection.relation_num; ++i) {
    want_relations.insert(selection.relations[i]);
  }
  for (size_t i = 0; i < selection.attr_num; ++i) {
    char *name = selection.attributes[i].relation_name;
    if (name != nullptr && want_relations.find(name) == want_relations.cend()) {
      rc = RC::SCHEMA_FIELD_NOT_EXIST;
    }
  }
  for (size_t i = 0; i < selection.attr_num; ++i) {
    const auto &condition = selection.conditions[i];
    if ((condition.left_is_attr && want_relations.find(condition.left_attr.relation_name) == want_relations.cend()) ||
        (condition.right_is_attr && want_relations.find(condition.right_attr.relation_name) == want_relations.cend()))
      rc = RC::SCHEMA_FIELD_NOT_EXIST;
  }
  if (rc != RC::SUCCESS) {
    session_event->set_response("FAILURE\n");
